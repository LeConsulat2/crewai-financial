...
# Define custom agent logic by extending the Agent class
class IncomeAgent(Agent):
    def __init__(self, role, goal, backstory, verbose=False):
        super().__init__(role=role, goal=goal, backstory=backstory, verbose=verbose)
...
my_crew = Crew(agents=[income_agent, expense_agent], tasks=[income_task, expense_task])
...
if uploaded_file:
    ...
    # Execute tasks
    crew_results = my_crew.kickoff(inputs={"input": pdf_text})
